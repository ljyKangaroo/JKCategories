...
+ (void)initialize
{
    _jk_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)jk_faceAwareFill {
    ...
    CGRect facesRect = [self jk_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self jk_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) jk_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _jk_faceDetector;
...
- (void) jk_scaleImageFocusingOnRect:(CGRect) facesRect {
